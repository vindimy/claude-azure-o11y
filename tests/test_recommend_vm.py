...
from recommend.ladder import fit_down, fit_up, next_smaller_in_family
...
def catalog(config_dir: Path) -> FamilySkuCatalog:
    return load_config(config_dir, "mg-x").catalog_for("vm", FamilySkuCatalog)
...
def test_memory_clause_raises_confidence(catalog: FamilySkuCatalog) -> None:
...
def test_never_crosses_family(catalog: FamilySkuCatalog) -> None:
...
def test_floor_allows_exactly_min_vcpu(catalog: FamilySkuCatalog) -> None:
...
def test_with_pricing_computes_saving(catalog: FamilySkuCatalog) -> None:
    rec = recommend_vm(finding("Standard_D8s_v5"), catalog, VmRecommendRules())
    priced = rec.with_pricing(Decimal("280.32"), Decimal("140.16"))
    assert priced.saving == Decimal("140.16")
    assert rec.with_pricing(Decimal("1"), None).saving is None
    assert "Pricing" not in rec.reason  # the pipeline appends the unpriced note, not the rule
...
def test_ladder_helpers() -> None:
    ...
    # 8 vCores at P95 20% with 1.3x headroom needs 2.08 -> the smallest size covering it is 4
    assert fit_down(ladder, 8, 20.0, 1.3) == 4
    assert fit_down(ladder, 8, 20.0, 1.3, floor=8) is None
    assert fit_down(ladder, 2, 20.0, 1.3) is None


def test_next_smaller_in_family_walks_the_catalog(catalog: FamilySkuCatalog) -> None:
    current = catalog.get("Standard_D8s_v5")
    assert current is not None
    assert next_smaller_in_family(catalog, current, 1) == "Standard_D4s_v5"
    assert next_smaller_in_family(catalog, current, 4) == "Standard_D4s_v5"
    assert next_smaller_in_family(catalog, current, 8) is None

...
- GitLab CI builds and pushes the image. `scripts/build-image.sh` does the same locally for a dev tag.

## Two independent paths

Both paths deploy the whole stack into an existing RG: an Elastic Premium plan, a Function App with the
UAMI attached and the image configured, app settings with Key Vault references, and the storage
containers. Both only **reference** an existing image tag. They never build, push, or zip-deploy
anything, and neither needs a Docker daemon.

- Neither path creates the RG, UAMI, storage account, Key Vault, or ACR. Those are inputs.
...
- Steps: validate inputs → confirm `image_tag` exists (`az acr repository show-tags`) → resolve the
  UAMI, storage, and KV IDs → create or update the plan and function app with `--image` and
  `--acr-identity` → attach the UAMI → set app settings → ensure containers → run
  `check-identity.sh` → print the invoke URL. Re-running with a new `image_tag` is the deploy.
- `destroy.sh` removes only what `deploy.sh` created (the plan and function app, plus containers on
  request).

### Path B: `terraform/module-azure-o11y` (CI, prod)

- Use the latest Terraform and `azurerm` 4.x. Keep separate `data.tf`, `locals.tf`, `variables.tf`,
...
| `storage_account_name` | `sto11yalerting` | existing; containers `reports` and `suppression` created if missing |

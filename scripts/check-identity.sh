...
while IFS=$'\t' read -r id role scope; do
  case ",$SKIP," in *",$id,"*) echo "skip  $id (not needed here)"; continue ;; esac
  placeholder=$scope
  ...
  # An input that was not given leaves an empty scope; the placeholder names what is missing.
  [[ -n "$scope" ]] || { echo "skip  $id (no $placeholder given)"; continue; }
